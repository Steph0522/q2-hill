...
plugin.methods.register_function(
    ...
    input_descriptions={
        ...
        "phylogeny": ("Rooted phylogenetic tree corresponding to species in the "
                      "table.")
    },
    parameter_descriptions={
        "q": "Order of Hill number (q ≥ 0).",
        "metric": ("Metric to calculate: 'PD' for hillR or 'qDT' for hilldiv2. "
                   "Default PD.")
    },
    output_descriptions={
    ...
    description=("Computes phylogenetic diversity using Hill numbers with a "
                 "given order q."),
)
...
plugin.methods.register_function(
    ...
    parameter_descriptions={
        "traits": ("Metadata containing trait data with species as rows and "
                   "trait variables as columns."),
        "q": "Order of Hill number (q ≥ 0).",
        "dist": ("Distance metric to compute: 'euclidean', 'manhattan' or "
                 "'gower'."),
        "metric": ("Metric to calculate: 'FD' for hilldiv2, or 'FD_q', 'D_q', "
                   "'Q', 'MD_q' or 'FDis' for hillR."),
        "tau": ("Threshold for distances. Can be a positive number or one of "
                "'min', 'max', or 'mean'.")
    },
    output_descriptions={
    ...
    description=("Computes functional diversity (Hill numbers) using a feature "
